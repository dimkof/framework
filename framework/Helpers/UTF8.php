...
class UTF8 {
	...
	public static function clean($var)
	{
		...
		elseif (is_string($var) AND $var !== '')
		{
			...
			if ( ! self::is_ascii($var))
			{
				...
				$var = iconv(self::CHARSET, self::CHARSET.'//IGNORE', $var);
	...
	public static function str_pad($str, $final_str_length, $pad_str = ' ', $pad_type = STR_PAD_RIGHT)
	{
		...
		throw new Simply_Exception("UTF8::str_pad: Unknown padding type (:pad_type)", array(
				':pad_type' => $pad_type,
			));
	}
	...
	public static function ord($chr)
	{
		...
		if ( ! isset($chr[1]))
		{
			throw new Simply_Exception('Short sequence - at least 2 bytes expected, only 1 seen');
		}
		...
		if ( ! isset($chr[2]))
		{
			throw new Simply_Exception('Short sequence - at least 3 bytes expected, only 2 seen');
		}
		...
		if ( ! isset($chr[3]))
		{
			throw new Simply_Exception('Short sequence - at least 4 bytes expected, only 3 seen');
		}
		...
		if ( ! isset($chr[4]))
		{
			throw new Simply_Exception('Short sequence - at least 5 bytes expected, only 4 seen');
		}
		...
		if ( ! isset($chr[5]))
		{
			throw new Simply_Exception('Short sequence - at least 6 bytes expected, only 5 seen');
		}
		...
		{
			throw new Simply_Exception("Invalid UTF-8 with surrogate ordinal ':ordinal'", array(
				':ordinal' => $ord0,
			));
		}
	}
		...
		for ($i = 0; $i < $len; $i++)
		{
			...
			else
			{
				...
				else
				{
					...
					throw new Simply_Exception("UTF8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ':byte'", array(
						':byte' => $i,
					));
				}
			}
	...
	public static function from_unicode($arr)
	{
		...
		foreach ($keys as $k)
		{
			...
			{
				// Found a surrogate
				throw new Simply_Exception("UTF8::from_unicode: Illegal surrogate at index: ':index', value: ':value'", array(
					':index' => $k,
					':value' => $arr[$k],
				));
			}
			...
			{
				throw new Simply_Exception("UTF8::from_unicode: Codepoint out of Unicode range at index: ':index', value: ':value'", array(
					':index' => $k,
					':value' => $arr[$k],
				));
			}
